...
* It's stable
* The base technologies are older then the syntax
...
== Futures are readiness-based

They will report once the operation is done and allow you query this state. The moment a future is ready, it _resolves_.
...
* Reference counting on single-threaded executors can be done using `Arc`
