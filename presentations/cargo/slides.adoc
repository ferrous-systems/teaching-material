...
`cargo test --all --examples` runs all test, including documentation test and compilation of examples!
...
Benchmarks only work on the nightly compiler.
...
- `1.0`: Any patch version of the "0.1" series
...
It can be considered to always keep the Lockfile under version control! (for repeatable builds)
