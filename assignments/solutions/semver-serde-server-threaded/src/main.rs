...
fn main() -> io::Result<()> {
    ...
    let repository = Arc::new(Mutex::new(EnumRegistry::new()));
    ...
    for connection in listener.incoming() {
        ...
        let repo_handle = repository.clone();
        std::thread::spawn(move || {
            let res = handle(&mut stream, &*repo_handle);

            if let Err(e) = res {
                if let Err(write_err) = write!(stream, "{}", e) {
                    println!("Could not write to stream: {:?}", write_err);
                }
                println!("Error occurred: {:?}", e);
            }
        });
...
fn handle(stream: &mut TcpStream, repository: &Mutex<EnumRegistry>) -> Result<(), ApiError> {
    let command = read_command(stream)?;
    // don't lock before you've read the stream!
